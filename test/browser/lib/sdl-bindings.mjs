...
export const routeRuntime = async page => {
	if(process.env.SDL_TEST_ARTIFACT_DIR)
	{
		...
		{
			const name = `php${version}_sdl-web.${suffix}`;
			await page.route(`**/${name}`, route => route.fulfill({
				path: resolve(process.env.SDL_TEST_ARTIFACT_DIR, name)
				, contentType: suffix === 'mjs' ? 'text/javascript' : 'application/wasm'
			}));
		}
	}
};
...
export const start = async page => {
	...
	await page.evaluate(async ({version, libType}) => {
