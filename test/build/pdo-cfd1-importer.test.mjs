...
const dockerMode = process.env.PDO_CFD1_IMPORTER_DOCKER === '1';
const image = process.env.PDO_CFD1_IMPORTER_IMAGE ?? 'php-wasm-vrzno-importer';
const extension = 'third_party/php8.3-src/ext/pdo_cfd1';
const stage = 'third_party/pdo-cfd1';
...
{
	const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'pdo-cfd1-importer-'));
	...
	const packageRelative = installed ? 'node_modules/pdo-cfd1' : 'packages/pdo-cfd1';
	const packagePath = path.join(workspace, packageRelative);
	fs.mkdirSync(packagePath, { recursive: true });
	if(installed)
	{
		const packed = JSON.parse(execFileSync('npm', ['pack', '--json', path.join(repoRoot, 'packages/pdo-cfd1'), '--pack-destination', temporary, '--cache', path.join(temporary, '.npm-cache')], {
	...
	const files = {
		'pdo_cfd1.c': '/* A */\n'
		, 'config.m4': 'dnl A\n'
		, 'php_pdo_cfd1.h': '/* header A */\n'
		, 'pdo_cfd1_db.c': '/* other A */\n'
		, 'obsolete.c': '/* obsolete */\n'
	...
	const dockerPrefix = ['run', '--rm', '-i', '--user', '0:0'
		...
		, '--mount', `type=bind,src=${repository},dst=/pdo-cfd1-fixture,readonly`
		, '-w', '/src', image];
	...
	if(dockerMode) builder('require("fs").cpSync("/pdo-cfd1-fixture","pinned-fixture",{recursive:true})');
...
.PHONY: all
all: runtime ${stage}/pdo_cfd1.c ${extension}/config.m4 ${extension}/pdo_cfd1.c
...
`);
	const run = ({ ref = a, source, origin, success = true, interrupt = false, goal = 'all', enabled = '1' } = {}) => {
		...
		const result = spawnSync('make', [
			...
			, goal
			, `DOCKER_RUN=${prefix}`
			, `WITH_PDO_CFD1=${enabled}`
			, `PDO_CFD1_REPOSITORY=${origin ?? (dockerMode ? '/src/pinned-fixture' : repository)}`
			, `PDO_CFD1_REF=${ref}`
			, ...(source ? [`PDO_CFD1_DEV_PATH=${source}`] : [])
	...
	return { workspace, repository, dev, a, b, run, read, write, remove, builder, mtimes, packageRelative };
}

test('real Make imports pinned sources and preserves all no-op mtimes and downstream build counts', t => {
	const f = fixture(t);
	f.run();
	assert.equal(f.read(`${extension}/pdo_cfd1.c`), '/* A */\n');
	assert.equal(f.read(`${extension}/CREDITS`), 'CFD1\nTest authors\n');
...
test('A to B to A selects actual source bytes and deletes only previously managed inputs', t => {
	...
	f.run({ ref: f.b });
	assert.equal(f.read(`${extension}/pdo_cfd1.c`), '/* A */\n');
	assert.equal(f.read(`${extension}/pdo_cfd1_db.c`), '/* other B */\n');
	assert.equal(f.read(`${extension}/php_pdo_cfd1.h`), '/* header B */\n');
	assert.equal(fs.existsSync(path.join(f.workspace, extension, 'obsolete.c')), false);
	assert.equal(f.read(`${extension}/added.c`), '/* added B */\n');
	f.run();
	assert.equal(f.read(`${extension}/pdo_cfd1.c`), '/* A */\n');
	assert.equal(f.read(`${extension}/pdo_cfd1_db.c`), '/* other A */\n');
	assert.equal(f.read(`${extension}/php_pdo_cfd1.h`), '/* header A */\n');
...
test('configure and every runtime track the manifest only while CFD1 is enabled', t => {
	const f = fixture(t);
...
test('pinned to older dev to same pin restores content; dev snapshots are external and read-only', t => {
	...
	f.run();
	f.run({ source: f.dev });
	assert.equal(f.read(`${extension}/pdo_cfd1.c`), '/* DEV */\n');
...
test('included C, header, config, README, addition and deletion changes refresh both inputs and downstream Make targets', t => {
	const f = fixture(t);
	f.run({ source: f.dev });
	for(const name of ['pdo_cfd1_db.c', 'php_pdo_cfd1.h', 'config.m4', 'config.w32', 'README.md'])
	{
		const before = f.read('build.log');
		const previous = fs.statSync(path.join(f.dev, name));
		const mainTime = fs.statSync(path.join(f.workspace, extension, 'pdo_cfd1.c'), { bigint: true }).mtimeNs;
		...
		assert.ok(fs.statSync(path.join(f.workspace, extension, 'pdo_cfd1.c'), { bigint: true }).mtimeNs > mainTime);
	}
...
test('unchanged requests repair tampered and missing staged/extension inputs', t => {
	const f = fixture(t);
	f.run();
	f.write(`${stage}/pdo_cfd1.c`, 'tampered staging');
	f.write(`${extension}/php_pdo_cfd1.h`, 'tampered extension');
	f.remove(`${extension}/config.m4`);
	f.run();
	assert.equal(f.read(`${stage}/pdo_cfd1.c`), '/* A */\n');
	assert.equal(f.read(`${extension}/php_pdo_cfd1.h`), '/* header A */\n');
...
test('failed pinned and development input leaves successful state and source contents unchanged', t => {
	...
	f.run({ origin: '/missing-pdo-cfd1-repository', success: false });
...
fs.renameSync = (source, destination) => {
  if(destination.endsWith('/ext/pdo_cfd1/pdo_cfd1.c')) throw new Error('Injected source-publication failure');
...
`);
	...
	f.write(`${extension}/.php-wasm-pending.json`, JSON.stringify({ schema: 1, files: ['pdo_cfd1.c', 'added.c'] }));
...
test('symlinked destinations and overlapping developer checkouts fail before overwrite', t => {
	...
	f.run();
	const state = f.read(`${stage}/${stateName}`);
	f.remove(`${extension}/pdo_cfd1.c`);
	f.builder('require("fs").symlinkSync("php_pdo_cfd1.h", process.argv[1])', `${extension}/pdo_cfd1.c`);
	f.run({ success: false });
	assert.equal(f.read(`${extension}/php_pdo_cfd1.h`), '/* header A */\n');
...
test('the published importer package includes its executable helper', t => {
	const cache = fs.mkdtempSync(path.join(os.tmpdir(), 'pdo-cfd1-pack-cache-'));
	t.after(() => fs.rmSync(cache, { recursive: true, force: true }));
	const packed = JSON.parse(execFileSync('npm', ['pack', '--dry-run', '--json', './packages/pdo-cfd1', '--cache', cache], {
...
test('CFD1 explicitly requires Vrzno when enabled', t => {
	const f = fixture(t);
	const result = spawnSync('make', ['--no-print-directory', 'dependencies', 'WITH_PDO_CFD1=1', 'WITH_VRZNO=0'], {
		cwd: f.workspace, encoding: 'utf8'
	});
	assert.notEqual(result.status, 0);
	assert.match(result.stderr, /WITH_PDO_CFD1=1 requires WITH_VRZNO=1/);
});
