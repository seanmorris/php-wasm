...
test('JS-only edits rebuild Vrzno and imports include locked build inputs without generated files', t => {
	...
	for(const directory of [stage, extension])
	{
		...
		for(const name of ['Makefile.frag', 'vrzno_js.h.in', 'vrzno_fetch_js.h.in', 'js/vrzno_init.js', 'js/php_stream_fetch_real_open.js', 'js/vrzno_weakermap.mjs', 'js/vrzno_bundle.mjs', 'package.json', 'package-lock.json', 'NOTICE'])
	...
	fs.appendFileSync(path.join(f.dev, 'js/vrzno_init.js'), '// JS-only edit\n');
	f.run({ source: f.dev });
	assert.equal(f.read('build.log'), 'configure\nbuild\n'.repeat(2));
	assert.match(f.read(`${extension}/js/vrzno_init.js`), /JS-only edit/);
