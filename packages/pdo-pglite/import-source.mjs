...
const sourceDirectory = 'third_party/pdo-pglite';
...
function extensionDirectory(version)
{
	...
	return `third_party/php${version}-src/ext/pdo_pglite`;
}
...
{
	if(snapshot?.schema !== 1 || !Array.isArray(snapshot.files)) throw new Error('Invalid PGlite source manifest');
	...
	{
		throw new Error('Invalid PGlite source identity');
	}
	const names = new Set;
	const files = snapshot.files.map(file => {
		if(!isInput(file.name) || names.has(file.name) || !/^[a-f0-9]{64}$/.test(file.sha256))
		{
			throw new Error('Invalid PGlite input inventory');
		}
		names.add(file.name);
	...
	for(const name of ['pdo_pglite.c', 'config.m4'])
	{
		if(!names.has(name)) throw new Error(`Missing required PGlite input: ${name}`);
	}
...
function developmentSnapshot(directory, version)
{
	...
	for(const relative of [sourceDirectory, extensionDirectory(version)])
	{
		...
		{
			throw new Error('PDO_PGLITE_DEV_PATH must not overlap an importer destination');
		}
	}
...
{
	if(!repository || !ref) throw new Error('PDO_PGLITE_REPOSITORY and PDO_PGLITE_REF are required');
	const cache = checkedPath(`.cache/pdo-pglite-import/${hash(repository)}`);
...
function readState(directory)
{
	...
	catch(error)
	{
		...
		process.stderr.write(`PGlite import: replacing invalid generated manifest ${filename}\n`);
		return null;
	}
}
...
function synchronize(relative, snapshot)
{
	...
	if(fs.existsSync(pendingPath))
	{
		...
		{
			throw new Error('Invalid pending PGlite input inventory');
		}
	}
...
async function main()
{
	...
	else if(command === 'sync')
	{
		...
		if(!state) throw new Error('PGlite staging manifest is missing');
...
main().catch(error => {
	process.stderr.write(`PGlite import failed: ${error.message}\n`);
	process.exitCode = 1;
});
