...
const sourceDirectory = 'third_party/vrzno';
...
const isInput = name => typeof name === 'string'
	...
	&& /^(?:.+\.(?:c|h)|.+\.stub\.php|config\.m4|CREDITS|LICENSE)$/.test(name);
...
function extensionDirectory(version)
{
	...
	return `third_party/php${version}-src/ext/vrzno`;
}
...
{
	if(snapshot?.schema !== 1 || !Array.isArray(snapshot.files)) throw new Error('Invalid Vrzno source manifest');
	...
	{
		throw new Error('Invalid Vrzno source identity');
	}
	const names = new Set;
	const files = snapshot.files.map(file => {
		if(!isInput(file.name) || names.has(file.name) || !/^[a-f0-9]{64}$/.test(file.sha256))
		{
			throw new Error('Invalid Vrzno input inventory');
		}
		names.add(file.name);
	...
	for(const name of ['vrzno.c', 'config.m4'])
	{
		if(!names.has(name)) throw new Error(`Missing required Vrzno input: ${name}`);
	}
...
function developmentSnapshot(directory, version)
{
	...
	for(const relative of [sourceDirectory, extensionDirectory(version)])
	{
		...
		{
			throw new Error('VRZNO_DEV_PATH must not overlap an importer destination');
		}
	}
...
{
	if(!repository || !ref) throw new Error('VRZNO_REPOSITORY and VRZNO_REF are required');
	const cache = checkedPath(`.cache/vrzno-import/${hash(repository)}`);
...
function readState(directory)
{
	...
	catch(error)
	{
		...
		process.stderr.write(`Vrzno import: replacing invalid generated manifest ${filename}\n`);
		return null;
	}
}
...
function synchronize(relative, snapshot)
{
	...
	if(fs.existsSync(pendingPath))
	{
		...
		{
			throw new Error('Invalid pending Vrzno input inventory');
		}
	}
...
async function main()
{
	...
	else if(command === 'sync')
	{
		...
		if(!state) throw new Error('Vrzno staging manifest is missing');
...
main().catch(error => {
	process.stderr.write(`Vrzno import failed: ${error.message}\n`);
	process.exitCode = 1;
});
