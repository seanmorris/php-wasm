...
const sourceDirectory = 'third_party/pdo-cfd1';
...
function extensionDirectory(version)
{
	...
	return `third_party/php${version}-src/ext/pdo_cfd1`;
}
...
{
	if(snapshot?.schema !== 1 || !Array.isArray(snapshot.files)) throw new Error('Invalid CFD1 source manifest');
	...
	const files = snapshot.files.map(file => {
		...
		{
			throw new Error('Invalid CFD1 input inventory');
		}
		names.add(file.name);
	...
	for(const name of ['pdo_cfd1.c', 'config.m4'])
	{
		if(!names.has(name)) throw new Error(`Missing required CFD1 input: ${name}`);
	}
	...
	if(identity.patch !== undefined) origin.patch = identity.patch;
...
function developmentSnapshot(directory, version)
{
	...
	for(const relative of [sourceDirectory, extensionDirectory(version)])
	{
		...
		{
			throw new Error('PDO_CFD1_DEV_PATH must not overlap an importer destination');
		}
	}
...
{
	if(!repository || !ref) throw new Error('PDO_CFD1_REPOSITORY and PDO_CFD1_REF are required');
	const cache = checkedPath(`.cache/pdo-cfd1-import/${hash(repository)}`);
...
function readState(directory)
{
	...
	catch(error)
	{
		...
		process.stderr.write(`CFD1 import: replacing invalid generated manifest ${filename}\n`);
		return null;
	}
}
...
function synchronize(relative, snapshot)
{
	...
	if(fs.existsSync(pendingPath))
	{
		...
		{
			throw new Error('Invalid pending CFD1 input inventory');
		}
	}
...
main().catch(error => {
	process.stderr.write(`CFD1 import failed: ${error.message}\n`);
	process.exitCode = 1;
});
