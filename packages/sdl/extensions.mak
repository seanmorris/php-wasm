...
# Changing feature flags must invalidate configure even when ENV_FILE is unchanged.
.PHONY: sdl-config-force
.cache/sdl-config-${PHP_VERSION}: sdl-config-force
	@mkdir -p .cache
	@printf '%s\n' '${WITH_SDL} ${WITH_SDL_IMAGE} ${WITH_SDL_MIXER} ${WITH_SDL_TTF} ${WITH_OPENGL}' > $@.tmp
	@cmp -s $@.tmp $@ && rm $@.tmp || mv $@.tmp $@

third_party/php${PHP_VERSION}-src/configured: .cache/sdl-config-${PHP_VERSION}
...
third_party/php$${PHP_VERSION}-src/ext/$(1)/config.m4: third_party/php$${PHP_VERSION}-$(1)/config.m4 packages/sdl/patches/$(1).patch packages/sdl/extensions.mak $(3) | third_party/php$${PHP_VERSION}-src/patched
...
$(eval $(call SDL_PECL,sdl_mixer,0.4.0,$(wildcard packages/sdl/mixer/*.h)))
$(eval $(call SDL_PECL,sdl_ttf,0.3.0))
$(eval $(call SDL_PECL,opengl,0.9.0,$(wildcard packages/sdl/opengl/*.[ch])))

define SDL_NATIVE
third_party/SDL2_$(1)-$(2)/configure:
