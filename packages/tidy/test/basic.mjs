...
test('Tidy Extension is enabled.', async () => {
	const php = env.WITH_TIDY === 'dynamic'
		? new PhpNode({sharedLibs:[
			{ url: `./packages/tidy/libtidy.so` },
			`./packages/tidy/php${process.env.PHP_VERSION ?? '8.4'}-tidy.so`,
		]})
		: new PhpNode;
...
test('Tidy Extension is enabled.', async () => {
	const php = env.WITH_TIDY === 'dynamic'
		? new PhpNode({sharedLibs:[
			{url: new URL(`../../tidy/libtidy.so`, import.meta.url) },
			new URL(`../../tidy/php${process.env.PHP_VERSION ?? '8.4'}-tidy.so`, import.meta.url),
		]})
		: new PhpNode;
